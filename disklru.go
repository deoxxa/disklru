...
type DiskLRU struct {
	Func    func(string, interface{}) ([]byte, error)
	MaxSize uint32
	db      *kv.DB
	path    string
	sync.Mutex
...
func New(path string, f func(string, interface{}) ([]byte, error), maxSize uint32) (*DiskLRU, error) {
	...
	db_path := filepath.Join(path, "index")
	createOpen := kv.Open
...
func (lru *DiskLRU) Get(key string, whatever interface{}) (data []byte, fetched bool, err error) {
	lru.Lock()
	defer lru.Unlock()
	...
	if lastAccessTime == 0 {
		defer lru.evict()
		fetched = true
		data, err = lru.Func(key, whatever)
		if err != nil {
			return
		}
